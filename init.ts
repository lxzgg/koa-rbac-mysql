...
interface menus {
    ...
    url: string,
    icon: string,
    menus: Array<{ name: string, url: string, icon: string, permissions: Array<{ name: string, url: string, resource: string }> }>,
}
...
function init(menus: Array<menus>, {rebuildTable, synchronize, mysql}: options) {
    if (!mysql.pool) {
